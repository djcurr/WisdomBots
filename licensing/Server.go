...
// licensing.CheckLicense("http://127.0.0.1:8080/presalebot", false, false)
...
func checkPresaleBotHandler(response http.ResponseWriter, request *http.Request) {
	request.ParseForm()
...
func checkTelegramBotHandler(response http.ResponseWriter, request *http.Request) {
	request.ParseForm()
	license := request.FormValue("license")
	var tmpexp string
	decrypted := licensing.Decrypt(KEY, license)
...
func API() {
	...
	router.HandleFunc("/presalebot", checkPresaleBotHandler).Methods("POST")
	router.HandleFunc("/telegrambot", checkTelegramBotHandler).Methods("POST")
	http.Handle("/", router)
	if SSL {
